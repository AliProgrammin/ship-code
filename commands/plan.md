...
> **Context rule:** Delegates all work to the `clean-planner` subagent. The main context only receives a commit summary.

# /clean:plan

Decompose a feature into atomic specs, execute them, and commit each one.

Usage: `/clean:plan <plain English description of what to build>`
...
6. **If gates pass:** commit with full traceability: `feat(clean-NNN): title / agent: claude-code / task: spec path / timestamp: ISO / scope: files`
7. **If gates fail:**
...
Open issues: see .clean/issues.md
